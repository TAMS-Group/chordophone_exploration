...
class OnsetDetector:
    ...
    def audio_cb(self, msg):
        ...
        if (self.buffer_time is not None) and (time_difference := now.to_sec()-self.buffer_time.to_sec()-(self.buffer.shape[0]/self.sr))> 0.005:
            rospy.logwarn(f"lost audio samples worth {time_difference}s. will drop remaining buffer and start over.")
            self.reset()
